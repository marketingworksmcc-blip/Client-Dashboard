...
import { TeamworkTaskListToggles } from "@/components/admin/TeamworkTaskListToggles";
import { isTeamworkConfigured, fetchProject, fetchTaskLists, fetchTimeEntries } from "@/lib/teamwork";
...
export default async function ClientTeamworkPage({
  ...
  const openTaskListCount = taskLists.filter((tl) => !tl.isComplete).length;
        ...
        <>
          {fetchError ? (
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <Card className="border-[#e2e0d9]">
                  <CardContent className="p-5">
                    <div className="flex items-start justify-between">
                      <div>
                        <p className="text-xs font-medium text-[#8a8880] uppercase tracking-wide mb-1">Task Lists</p>
                        <p className="text-2xl font-semibold text-[#464540] font-heading">{taskLists.length}</p>
                        <p className="text-xs text-[#8a8880] mt-1">{openTaskListCount} in progress</p>
                      </div>
              ...
              {/* Task lists with visibility toggles */}
              <Card className="border-[#e2e0d9]">
                <CardHeader className="pb-3">
                  <CardTitle>Project Task Lists</CardTitle>
                  <p className="text-xs text-[#8a8880] mt-0.5">
                    Control which task lists are visible to the client.
                  </p>
                </CardHeader>
                <CardContent>
